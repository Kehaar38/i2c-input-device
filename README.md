...
- 動作テスト用デバイスも製作（[Test Device](#Test-Device-(XIAO-ESP32S3))）
...
- Byte0: エンコーダー差分（int8_t）
  - 前回 Read 以降の差分
    ...
    - Read したタイミングで累積差分を 0 にリセット（差分式）
...
# Test Device (XIAO ESP32S3)

## 概要
- I2C Input Deviceの出力をテストするデバイス
- ボタンの押下状況とエンコーダーの累積値を表示する
